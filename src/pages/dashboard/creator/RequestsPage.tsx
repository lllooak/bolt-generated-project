...
export function RequestsPage() {
  ...
  // Function to get fan name
  const getFanName = async (fanId: string): Promise<string> => {
    try {
      const { data, error } = await supabase
        .from('users')
        .select('name, email')
      ...
      if (error) throw error;
      return data?.name || data?.email?.split('@')[0] || 'מעריץ';
    } catch (err) {
  ...
  const handleStatusUpdate = async (requestId: string, newStatus: string) => {
    try {
      ...
      // If status is changed to approved, send notification email to creator
      if (newStatus === 'approved') {
        const request = requests.find(r => r.id === requestId);
        if (request) {
          const creatorEmail = await getCreatorEmail(request.creator_id);
          const fanName = await getFanName(request.fan_id);
          
          if (creatorEmail) {
            await sendCreatorOrderNotificationEmail(
              requestId,
              creatorEmail,
              request.creator_name || 'יוצר',
              fanName,
