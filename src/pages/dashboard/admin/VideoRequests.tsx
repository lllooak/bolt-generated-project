...
import { sendCreatorOrderNotificationEmail } from '../../../utils/emailService';
...
export function VideoRequests() {
  ...
  async function updateRequestStatus(requestId: string, newStatus: string) {
    try {
      ...
      // Get creator email if status is changing to approved
      let creatorEmail = null;
      if (newStatus === 'approved') {
        creatorEmail = await getCreatorEmail(requestData.creator_id);
      }
        ...
        if (creatorEmail) {
          try {
            ...
            const { data: fanData } = await supabase
              .from('users')
              .select('name, email')
              .eq('id', requestData.fan_id)
              .single();

            const fanName = fanData?.name || fanData?.email?.split('@')[0] || 'מעריץ';
            ...
            await sendCreatorOrderNotificationEmail(
              requestId,
              creatorEmail,
              (await getCreatorName(requestData.creator_id)) || 'יוצר',
              fanName,
              fullRequestData?.request_type || 'video_request',
              fullRequestData?.message || '',
              requestData.price
            );
          } catch (emailError) {
      ...
      // For other status changes
        ...
        // Send notification email to creator if status is approved
        if (newStatus === 'approved' && creatorEmail) {
          try {
            // Get fan name
            const { data: fanData } = await supabase
              .from('users')
