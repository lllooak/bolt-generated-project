...
export function InboxPage() {
  ...
  const generateMockNotifications = (): Notification[] => {
    ...
    return [
      {
        id: '1',
        title: 'הוידאו שלך מוכן!',
        message: 'הוידאו שהזמנת מאת שרה זמרת מוכן לצפייה',
      ...
      {
        id: '2',
        title: 'בקשת הוידאו אושרה',
        message: 'בקשת הוידאו שלך אושרה ע"י דני קומיקאי',
      ...
      {
        id: '3',
        title: 'עדכון יתרת ארנק',
        message: 'טעינת הארנק שלך בסך ₪200 הושלמה בהצלחה',
  ...
  const handleNotificationClick = async (notification: Notification) => {
    ...
    if (notification.type === 'request') {
      ...
      if (notification.entity_id) {
        handleChatSelect(notification.entity_id, 'יוצר');
      }
    }
  };
          ...
          {filter !== 'notifications' && (
            (filter === 'all' ? filteredItems.chats : filteredItems).map(([chatId, chatMessages]) => {
              ...
              const chatPartnerName = isOutgoing 
                ? lastMessage.receiver_name || 'יוצר' 
                : lastMessage.sender_name || 'יוצר';
