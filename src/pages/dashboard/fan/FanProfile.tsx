...
import { User, Calendar, Mail, Phone, Camera } from 'lucide-react';
  ...
  async function handleAvatarUpload(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (file.size > 2 * 1024 * 1024) {
        toast.error('גודל הקובץ חייב להיות קטן מ-2MB', { id: 'upload' });
        return;
      }

      // Validate file type
      if (!['image/jpeg', 'image/png', 'image/webp', 'image/gif'].includes(file.type)) {
        toast.error('סוג הקובץ חייב להיות JPEG, PNG, WEBP או GIF', { id: 'upload' });
        return;
      }
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-base text-gray-900">{profile?.phone || 'לא הוגדר'}</p>
              </div>
            </div>
