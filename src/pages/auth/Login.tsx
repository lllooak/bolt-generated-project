...
export function Login() {
  ...
  const handlePasswordReset = async (e: React.MouseEvent) => {
    ...
    if (!/\S+@\S+\.\S+/.test(formData.email)) {
      setErrors({ ...errors, email: 'אנא הזן כתובת אימייל תקינה' });
      return;
    }
    ...
    try {
      ...
      if (!result.success) {
        if (result.error.message.includes('rate limit')) {
          ...
          toast.error('נא להמתין 30 שניות לפני נסיון נוסף');
        } else {
          toast.error('שגיאה בשליחת אימייל לאיפוס סיסמה');
        }
      } else {
        toast.success('נשלח אימייל עם הוראות לאיפוס סיסמה');
        setIsResettingPassword(false);
      }
    } catch (error) {
      console.error('Password reset error:', error);
      toast.error('אירעה שגיאה בלתי צפויה. אנא נסה שוב מאוחר יותר.');
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div>
            {isResettingPassword ? (
              ...
              >
                {isLoading ? 'שולח...' : cooldownTime > 0 ? `אנא המתן ${cooldownTime} שניות` : 'שלח קישור לאיפוס סיסמה'}
              </button>
            ) : (
              <button
                type="submit"
