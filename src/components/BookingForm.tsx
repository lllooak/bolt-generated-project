...
export function BookingForm({ 
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isNaN(deadlineDate.getTime())) {
      toast.error('תאריך היעד אינו תקין');
      return;
    }
    
    if (deadlineDate < today) {
      toast.error('תאריך היעד חייב להיות בעתיד');
      return;
    }
    
    // Submit form with validated deadline
    onSubmit({
      ...formData,
      deadline: format(deadlineDate, 'yyyy-MM-dd') // Ensure consistent date format
    });
  };
      ...
      <div className="bg-gray-50 p-4 rounded-lg">
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            <span className="text-lg font-semibold text-gray-900">₪{(price || 0).toFixed(2)}</span>
          </div>
        </div>
      </div>
